...
def calculate_remaining_time(duration_seconds, resume_duration):
    current_time = rtc.datetime()

    # Adjust duration based on pause duration
    duration_seconds -= resume_duration
    print(f"Duration seconds: {duration_seconds} | Resume Duration: {resume_duration}")
    ...
    end_time_minutes = current_time[5] + duration_minutes
    end_time_hours = current_time[4] + duration_hours

    # Calculate carryover for seconds and minutes
    carry_minutes, end_time_seconds = divmod(end_time_seconds, 60)
    end_time_minutes += carry_minutes

    carry_hours, end_time_minutes = divmod(end_time_minutes, 60)
    end_time_hours += carry_hours

    # Adjust hours to fit within 24-hour format
    end_time_hours %= 24
...
def pause_resume(duration_input):
    ...
    resume_start_time = rtc.datetime()
    ...
    while True:
        ...
        if button_state == 0:
            if not paused:
                ...
                resume_end_time = rtc.datetime()
                resume_duration += (
                    (resume_end_time[4] - resume_start_time[4]) * 3600 +
                    (resume_end_time[5] - resume_start_time[5]) * 60 +
                    (resume_end_time[6] - resume_start_time[6])
                    )
                ...
                resume_start_time = rtc.datetime()
                paused = False
                end_time = calculate_remaining_time(duration_input, resume_duration)
                print(f"Resumed")
        ...
        utime.sleep(1)  # Small delay to avoid high CPU usage
